...
def run_sequence(seq: pp.Sequence, phantom, size, title="Result", plot=True):
    """Run a sequrnc on a MR0 phantom

    Args:
        seq (pp.Sequence): Pulseq sequence
        phantom (MRzeroCore.Phaser): MR0 phantom object
        size (tuple): Size of the imaging volume
        title (str, optional): Title for the plot. Defaults to "Result".
...
def get_event_centers(seq, verbose=True):
    """Get the center times of RF and ADC events in the sequence.

    Args:
        seq (pp.Sequence): The sequence to analyze.
        verbose (bool, optional): Whether to print the event centers. Defaults to True.

    Returns:
        list: A list of event centers with their types and timings.
    """
    cumulative_time = 0
    event_centers = []
    ...
    if verbose:
        ...
        for event in event_centers:
            print(
                f"{event['type']} (Block {event['block']}): {event['center']*1000:.3f} ms"
            )
            if event["type"] == "RF":
...
def check_sequence_timing(seq: pp.Sequence):
    ...
    Args:
        seq (pp.Sequence): The sequence to check.
    """
